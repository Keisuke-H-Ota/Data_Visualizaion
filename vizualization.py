...
text = '''# 売上データの可視化
[Kaggle](https://www.kaggle.com/aungpyaeap/supermarket-sales?select=supermarket_sales+-+Sheet1.csv)に登録されている売上データの可視化に取り組んだ。このデータセットには支店 A、B、C の売上データが含まれているため支店間の売上を比較することを目的とする。
'''
st.markdown(text)
...
if norm == 'Yes':  
    stacked_bar = alt.Chart(df).mark_bar(size=35).encode(
        x=alt.X('sum(cogs)',axis=alt.Axis(labelFontSize=20, ticks=True, titleFontSize=20, labelAngle=0),stack="normalize", scale=alt.Scale(domain=[0,1,0])),
...
line = alt.Chart().mark_point().encode(
...
band = alt.Chart().mark_errorbar(extent='stderr',ticks=True,orient='vertical').encode(
    x=alt.X(axis, axis=alt.Axis(labelFontSize=20, ticks=True, titleFontSize=20, grid=False),sort=sort),
